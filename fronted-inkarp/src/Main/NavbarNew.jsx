      ...
      if (p && typeof p === "object" && p.name) {
        ...
        if (Array.isArray(p.models)) {
          p.models.forEach((m) => {
            ...
            const keywordMatch = Array.isArray(m.keywords)
              ? m.keywords.some((kw) =>
...
export default function NavbarNew() {
  ...
  return (
    <>
      <div
        ref={navRef}
        className="flex flex-col h-screen max-h-screen w-full py-4 px-3 space-y-3 overflow-y-auto"
      >
        {/* Logo (desktop only) */}
        ...
        <nav className="flex-1 space-y-2 font-medium mt-15 lg:mt-0">
          ...
          <Link
            to="/careers"
            className={`block px-3 py-2 rounded-md transition-all duration-200 ${isActive("/careers")
              ? "bg-[#BE0010] text-white shadow"
              : "text-black hover:bg-gray-300"
              }`}
            onClick={() => {
          ...
          <Link
            to="/contactUs"
            className={`block px-3 py-2 rounded-md transition-all duration-200 ${isActive("/contactUs")
              ? "bg-[#BE0010] text-white shadow"
              : "text-black hover:bg-gray-300"
              }`}
            onClick={() => {
