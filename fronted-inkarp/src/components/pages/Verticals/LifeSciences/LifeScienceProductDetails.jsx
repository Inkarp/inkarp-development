...
// ---- Data source (Life Sciences)
import { NanosurfProducts } from "../../../../data/LifeScience/NanosurfProducts";
import { BwbProducts } from "../../../../data/LifeScience/BwbProducts";
import EnquiryForm from "../../../EnquiryFrom";

// --- product map (keys by productSlug)
const LifeScienceMap = {
  ...NanosurfProducts,
  ...BwbProducts
};


const combinedHeadingProducts = [
  "P4PRO+",
  "P4SPR (Portable Surface Plasmon Resonance)",
];


const productLinks = {
  nanosurf: "https://www.nanosurf.com/en/",
  bwbtechnologies: "https://www.bwbtech.com/",
  implen: "https://www.implen.de/",
  reichert: "https://www.reichertspr.com/",
  anvajo: "https://anvajo.com/",
  photon: "https://www.photonetc.com/",
  nenovision: "https://www.nenovision.com/",
  'affinite instruments' :"https://www.affiniteinstruments.com"
};

export default function LifeScienceProductDetails() {
  const [showEnquiry, setShowEnquiry] = useState(false);
  const { type, productSlug } = useParams();
  const product = LifeScienceMap[productSlug];
  const meta = product?.meta;
  const [showModal, setShowModal] = useState(false);

const isCombinedMode =
  combinedHeadingProducts.includes(product.name);
  ...
  // Share URL (Life Sciences base)
  const baseUrl = `${window.location.origin}/verticals/life-sciences`;
        ...
        <div className="block md:hidden bg-gray-50 p-4 rounded-xl border">
          <div className="flex items-center gap-4">
            ...
            <div className="w-2/3 space-y-2">
              ...
              <h1 className="text-sm font-semibold font-[MaxOT]  leading-tight">
                <a
        ...
        <div className="hidden md:block relative w-full">
          ...
          <div className="absolute inset-0 flex flex-row items-center justify-around p-4">
            ...
            <div className="w-1/2 text-center">
              ...
              <h1 className="text-3xl font-bold tracking-tight font-[MaxOT] text-black drop-shadow-md">
                <a
                  href={productLinks[product.altText?.toLowerCase()] || "#"}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      {Array.isArray(product.features) && product.features.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
          <div className="bg-[#F5F5F5] p-8 rounded-3xl shadow-xl border border-gray-200 hover:shadow-2xl transition duration-300">
            <h2 className="text-3xl  text-center text-[#E63946] font-[MaxOT] mb-6">
              {isCombinedMode
                ? `Key Features & Advantages of ${product.name}`
                : `Key Features of ${product.name}`}
            </h2>
            {product.features[0]?.overview && (
              <p className="text-center text-gray-700 text-base md:text-lg mb-8 font-[Roboto]">
                {product.features[0].overview}
              </p>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {product.features
                .filter((f) => f.name) // removes overview object from grid
                .map((feature, i) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-800 mt-1">{feature.description}</p>
                    </div>
                  </div>
                ))}
            </div>
          ...
          {Array.isArray(product.advantages) && product.advantages.length > 0 && (
            <div className="bg-[#F5F5F5] p-8 rounded-3xl shadow-xl border border-gray-200 hover:shadow-2xl transition duration-300">
              <h2 className="text-3xl  text-center text-[#E63946] font-[MaxOT] mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {product.advantages.map((advantage, i) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold">{advantage.name}</p>
                      <p className="text-sm text-gray-800 mt-1">{advantage.description}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {product.applications.map((application, i) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-800 mt-1">{application.description}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {product.techSpecs && (
        <div className="p-5 w-full max-w-7xl mx-auto">
          <div className="bg-[#F5F5F5] shadow-xl overflow-hidden rounded-3xl px-2 border border-gray-200">
            <h2 className="text-3xl  mt-5 text-[#E63946] text-center font-[MaxOT]">
              Technical Specifications of {product.name}
            </h2>
            {product.techSpecs[0]?.overview && (
              <p className="text-center text-gray-700 text-base md:text-lg mb-8 font-[Roboto]">
                {product.features[0].overview}
              </p>
            )}
