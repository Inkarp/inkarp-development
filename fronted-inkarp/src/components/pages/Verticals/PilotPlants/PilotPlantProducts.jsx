...
import { PilotPlants } from "../../../../data/PilotPlants";

const synthesisData = [
  {
    principal: "AP MiniPlant",
    logo: AP,
    products: ["Stirred Reactor Plants", "Catalyst Testing Plant"],
  },
  {
    principal: "Chemspeed",
    logo: Chemspeed,
...
export default function PilotPlantProducts() {
  const navigate = useNavigate();
  const { type = "stirred-reactor-plants" } = useParams();
  ...
  // --- match behavior from the provided component ---
  const normalize = (s) =>
    (s || "")
      .toString()
      .toLowerCase()
  ...
  const getPrincipalLogoByType = (t) => {
    ...
    return AP; // safe fallback on this page
  };
  ...
  return (
    <>
      <div className="flex w-full h-full overflow-hidden">
        ...
        {/* Main Content — aligned to the provided component style */}
        <main className="w-full h-full overflow-y-auto py-2 px-5 font-[Roboto]">
          ...
          <div className="p-6 max-w-6xl mx-auto">
            ...
            {pageData.highlight && <p>{pageData.highlight}</p>}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-3 justify-items-center">
              {products.map((product, index) =>
                product.slug ? (
                  <Link
                    key={product.slug || index}
                      to={`/verticals/pilot-plants-and-automation/${type}/${product.slug}`}
