...
import { DownloadIcon, Share2, ZoomIn } from "lucide-react";
import { HiOutlineClipboardList } from "react-icons/hi";

import { Battery } from "../../../../data/MaterialScience/Battery";
import EnquiryForm from "../../../EnquiryFrom";

import Catalog from "/src/assets/LabstationCatalog.pdf"

// Merge all product sets you want available on this page
const SynthesisProductDetails = {
  ...Battery,
};

// --- helpers ---
const isHtmlString = (val) =>
  typeof val === "string" && /<\/?[a-z][\s\S]*>/i.test(val.trim());

const toArray = (val) => (Array.isArray(val) ? val : [val]);

export default function MaterialScienceProductDetails() {
  ...
  if (!product) {
    return (
      <div className="p-10 text-center bg-white rounded-lg shadow-xl">
        <h2 className="text-2xl font-bold text-red-600 font-[MaxOT]">
          Product Not Found
        </h2>
  ...
  // Normalize product.description so the UI can safely render both string & array forms
  // - If it's a string (e.g., an HTML table block), wrap it as a single section
  // - If it's already an array of sections, use as-is
  const sections = Array.isArray(product.description)
    ? product.description
    : product.description
      ? [{ heading: "Details", description: product.description }]
      : [];
    ...
    <>
      ...
      <div className="bg-white px-2 sm:px-5 py-4">
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4">
          {product.name === "Labstation I – Glovebox and Accessories" && (
            <a
              href={Catalog}                  
              download
              target="_blank"
            ...
            >
              <button className="flex items-center justify-center gap-2 bg-[#E63946] text-white px-5 py-2.5 rounded-full hover:bg-red-700 transition-transform transform hover:scale-105 shadow-lg font-semibold font-[MaxOT]">
                <DownloadIcon size={18} /> Download Catalogue
              </button>
            </a>
          )}
          {/* <a
        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-12">
            ...
            <div className="flex flex-col items-center w-full">
              ...
              <div className="space-y-6 w-full mx-auto">
                {sections.map((section, idx) => (
                  <div key={idx}>
                    {section.heading && (
                      <h3 className="text-xl text-[#E63946] font-[MaxOT] mb-1">
                        {section.heading}
                      </h3>
                    )}

                    {/* Support: string, HTML string, or array of blocks */}
                    {Array.isArray(section.description)
                      ? toArray(section.description).map((block, i) =>
                        isHtmlString(block) ? (
                          <div
                            key={i}
                            className="prose max-w-none font-[Roboto]"
                            dangerouslySetInnerHTML={{ __html: block }}
                          />
                        ) : (
                          <p
                            key={i}
                            className="text-base font-[Roboto] leading-relaxed whitespace-pre-line"
                          >
                            {block}
                          </p>
                        )
                      )
                      : isHtmlString(section.description)
                        ? (
                          <div
                            className="prose max-w-none font-[Roboto]"
                        ...
                        : (
                          <p className="text-base font-[Roboto] leading-relaxed whitespace-pre-line">
                            {section.description}
                          </p>
                        )}
                  </div>
          ...
          {/* Image Gallery (if only images are provided) */}
          {product.imagesOnly && product.imagesOnly.length > 0 && (
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              <h3 className="text-2xl font-bold text-center text-[#E63946] mb-6 font-[MaxOT]">
                Product Image Gallery
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {product.imagesOnly.map((img, idx) => (
                  <img
                    key={idx}
                    src={img}
                    alt={`${product.name} image ${idx + 1}`}
                    className="w-full h-auto object-cover rounded-lg shadow hover:scale-105 transition"
                  />
                ))}
              </div>
            </div>
          ...
          {/* Technical Specifications (if specs exist) */}
                  ...
                  {product.features.map((feature, index) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col w-full gap-1">
                        ...
                        {feature.description && (
                          <p className="text-sm sm:text-base whitespace-pre-line font-[Roboto] text-gray-600">
                            {feature.description}
                          </p>
                        )}
                      </div>
