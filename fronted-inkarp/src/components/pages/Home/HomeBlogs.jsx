...
export default function HomeBlogs() {
  // 1) Latest 3 by date (newest first)
  const latestThree = useMemo(() => {
    return [...(articles || [])]
      .filter(a => a?.date) // safety
      .sort((a, b) => new Date(b.date) - new Date(a.date))
      .slice(0, 3);
  }, []);

  // 2) Initialize featured from the newest
  const initialFeatured = latestThree[0] || (articles?.[0] ?? null);
  const [featured, setFeatured] = useState(initialFeatured);
  const [activeId, setActiveId] = useState(initialFeatured?.id);
  ...
  const handleReadMore = (item) => {
    navigate(`/insights-and-updates/blogs/${item.slug}`, { state: { card: item } });
  };
  ...
  return (
    <section className="relative w-[98%] mx-auto p-5">
      ...
      <div className="text-center flex flex-col justify-center items-center gap-3">
        ...
        <h1 data-animate className="text-xl sm:text-2xl font-[MaxOT] text-[#E63946] leading-tight text-center">
      ...
      <div className="lg:hidden mt-5 relative max-w-xl mx-auto">
        ...
        >
          <span className="font-[Roboto] text-[#E63946] line-clamp-1">
            {featured?.title || "Select an article"}
          </span>
          {mobileOpen ? <ChevronUp size={50} className="text-[#E63946]" /> : <ChevronDown size={50} className="text-[#E63946]" />}
        </button>
                    ...
                    <li key={item.id}>
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 border-b last:border-b-0 transition ${
                          isActive ? "bg-[#F5F5F5] text-[#E63946] font-semibold" : "hover:bg-[#E63946] hover:text-white"
                        }`}
                      >
      ...
      {/* Content frame */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mt-6 rounded-3xl p-[1.5px] border border-gray-200">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          {/* LEFT – Blog list */}
          <div className="lg:col-span-6 hidden lg:block">
            <div className="rounded-2xl p-4 sm:p-5">
              <div className="flex flex-col gap-3 items-stretch justify-start overflow-y-auto custom-scroll max-h-[70vh]">
                {latestThree.map((item) => {
                  const isActive = activeId === item.id;
                  return (
                    <motion.button
                      ...
                      className={`text-left rounded-xl border p-3 transition w-full ${
                        isActive
                          ? "bg-[#F5F5F5] border-[#E63946] shadow-md text-[#E63946]"
                          : "bg-white text-black border-gray-200 hover:bg-[#E63946] hover:text-white"
                      }`}
                      aria-pressed={isActive}
                    >
                      <h4 className="text-sm sm:text-base font-[MaxOT] text-center">{item.title}</h4>
                      <div className="flex items-center justify-center gap-2 text-xs sm:text-sm mt-1">
                        <span className="inline-flex items-center text-black gap-1">
                          <CalendarDays size={14} />
                          {new Date(item.date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
                        </span>
                      </div>
          ...
          <div className="lg:col-span-6">
            <motion.div key={activeId} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.35 }} className="p-4 sm:p-5">
